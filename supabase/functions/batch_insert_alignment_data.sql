-- 删除旧函数（如果存在）
DROP FUNCTION IF EXISTS batch_insert_alignment_data(jsonb);

-- 创建批量插入对齐数据的数据库函数
-- 这个函数将大幅提升性能，避免多次网络往返

CREATE OR REPLACE FUNCTION batch_insert_alignment_data(alignment_data jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 为函数添加注释
COMMENT ON FUNCTION batch_insert_alignment_data(jsonb) IS '
批量插入音频对齐数据的高性能函数。

参数结构:
{
  "speechId": "string (UUID格式)",
  "blocks": [
    {
      "blockId": "string (UUID格式)", 
...
-- 授予执行权限 (根据你的权限模型调整)
-- GRANT EXECUTE ON FUNCTION batch_insert_alignment_data(jsonb) TO authenticated; 