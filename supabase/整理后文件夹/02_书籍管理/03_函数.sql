...
-- 章节删除后清理内容父级
CREATE OR REPLACE FUNCTION public.clean_content_parents_after_chapter_delete()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
    -- 删除content_parents表中的相应记录
    DELETE FROM content_parents WHERE id = OLD.parent_id;
    RETURN OLD;
END;
$function$

-- 页面删除后清理内容父级
CREATE OR REPLACE FUNCTION public.clean_content_parents_after_page_delete()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
...
-- 删除指定位置的章节
CREATE OR REPLACE FUNCTION public.delete_chapter_at_position(p_book_id uuid, p_position integer)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
-- 在指定位置插入章节
CREATE OR REPLACE FUNCTION public.insert_chapter_at_position(p_book_id uuid, p_position integer, p_title text, p_user_id uuid)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
-- 重新排序章节
CREATE OR REPLACE FUNCTION public.reorder_chapter(p_book_id uuid, p_from_index integer, p_to_index integer)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
-- 更新书籍笔记计数
CREATE OR REPLACE FUNCTION public.update_book_note_count()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
BEGIN
